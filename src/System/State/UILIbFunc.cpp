...
float GetStringWidth(Font font, const char *text, float fontSize, float spacing)
{
    ...
    for (int i = 0; text[i] != '\0';)
    {
        ...
        // Advance the pointer by the number of bytes read for the current codepoint
        i += codepointByteCount;

        // Calculate glyph width (consider advanceX or rec width if advanceX is zero)
        float glyphWidth = (font.glyphs[glyphIndex].advanceX == 0) ? 
                           font.recs[glyphIndex].width * scaleFactor : 
                           font.glyphs[glyphIndex].advanceX * scaleFactor;

        // Add glyph width and spacing to the total width
        width += glyphWidth;

        // Add spacing between characters
        if (text[i] != '\0') width += spacing;
    }
